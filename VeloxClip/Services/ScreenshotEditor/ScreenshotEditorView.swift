...
struct ScreenshotEditorView: View {
    ...
    private func renderEditedImage() -> NSImage {
        // Compositing lives in AnnotationRenderer: it is pure, and the Retina
        // sizing / coordinate-flip / mosaic-mirroring math it carries is exactly
        // the kind that regresses silently inside a view.
        AnnotationRenderer.render(image: image, elements: editorState.elements)
    }
    ...
    private func drawMosaicEffectInCanvas(context: GraphicsContext, rect: CGRect, imageOrigin: CGPoint, scaleX: CGFloat, scaleY: CGFloat) {
        ...
        if let pixelatedCGImage = AnnotationRenderer.pixelatedPatch(of: image, forPointRect: imageRect) {
